...
df = pd.read_csv('Plik_wyniki.csv', sep=';')
pd.options.display.max_rows = 10

X = df['x'].round(5)
Y = df['y'].round(5)
Z = df['current-flux-density-magnitude'].round(5)
...
def Wieght_average_of_points():
    ...
    for key in New_dict1:
        ...
        operator.drop(columns = ['z', 'Unnamed: 4'], inplace=True) 
...
def Average_weight():
    ...
    for keys in dictionary:
        ...
        weighted_average = sum(operator['current-flux-density-magnitude']*operator['weight'])/operator['weight'].sum()
